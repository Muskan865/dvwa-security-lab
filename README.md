...
![CSRF High](images/csrf/csrf_high.png)

Explanation:

At the High security level, DVWA uses a CSRF token called user_token. This token is generated by the server and must be included in the request. Since the malicious request did not contain the correct token, the server rejected the request and the attack failed. However, the token can still be obtained by inspecting the page source or checking the browser console. After retrieving the token, it can be inserted into the crafted request URL. Because the request now contains a valid token, the server accepts it and the password change is completed.
...
![File Upload High](images/file_upload/fu_high_updated.png)

Explanation:

At the High security level, the application applies stricter validation on uploaded files. It checks the file extension, MIME type, and file content, and also verifies image properties such as image size to ensure that only legitimate images are uploaded. Despite these protections, it is still possible to bypass the checks by creating a polyglot file. This is done by placing valid image magic bytes at the beginning of the file so that it appears to be a legitimate image, while still containing executable PHP code. As a result, the application accepts the malicious file because it passes the validation checks.
